 ...
 * @var $model          app\models\TasksHasNodes
...
$page_name   = ($action == 'assign-node-task') ? Yii::t('network', 'Assign task to node') : Yii::t('network', 'Edit node assignment');
    ...
    <div class="col-md-8 col-md-offset-2">
        <div class="box box-default">
            <div class="box-header with-border">
                <h3 class="box-title">
                    <i class="fa <?= ($action == 'assign-node-task') ? 'fa-plus' : 'fa-pencil-square-o' ?>"></i> <?= $page_name ?>
                </h3>
            </div>
            <?php
            ...
            ?>
                <div class="box-body">
                    <?php
                        echo $form->field($model, 'node_id')->dropDownList($model->selected_node, [
                            'class'            => 'select2-ajax',
                            'data-placeholder' => Yii::t('network', 'Enter node hostname or IP'),
                            'data-url'         => Url::to(['ajax-get-nodes'])
                        ]);
                ...
                <div class="box-footer text-right">
                    <?php
                        if ($action == 'edit-node-task') {
                            echo Html::a(Yii::t('app', 'Delete'), ['delete-node-task', 'id' => $model->id], [
                                'class' => 'btn btn-sm btn-danger pull-left',
                                'data' => [
                                    'confirm' => Yii::t('app', 'Are you sure you want to delete record {0} {1}?', [$model->node->hostname, $model->node->ip]),
                                    'method'  => 'post'
                                ],
                            ]);
                        }
